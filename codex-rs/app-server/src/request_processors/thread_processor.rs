...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_runtime_cwd_update(
        ...
        self.thread_runtime_cwd_update_inner(request_id, params)
            .await
            .map(|response| Some(response.into()))
    }
    ...
    async fn thread_runtime_cwd_update_inner(
        &self,
        request_id: &ConnectionRequestId,
        params: ThreadRuntimeCwdUpdateParams,
    ) -> Result<ThreadRuntimeCwdUpdateResponse, JSONRPCErrorError> {
        ...
        Ok(ThreadRuntimeCwdUpdateResponse {})
    }
