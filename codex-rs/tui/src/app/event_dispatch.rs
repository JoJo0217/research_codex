    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::MarketplaceAddLoaded {
                ...
                if add_succeeded && is_current_discovery {
                    if let Err(err) = self
            ...
            AppEvent::MarketplaceUpgradeLoaded { cwd, result } => {
                ...
                let is_current_discovery =
                    self.chat_widget.discovery_cwd().as_path() == cwd.as_path();
                if marketplace_contents_changed && is_current_discovery {
                    if let Err(err) = self
                        .refresh_in_memory_config_from_disk_for_discovery_cwd(cwd.clone())
                        .await
                    {
            ...
            AppEvent::MarketplaceRemoveLoaded {
                ...
                let is_current_discovery =
                    self.chat_widget.discovery_cwd().as_path() == cwd.as_path();
                if remove_succeeded && is_current_discovery {
                    if let Err(err) = self
                        .refresh_in_memory_config_from_disk_for_discovery_cwd(cwd.clone())
                        .await
                    {
            ...
            AppEvent::SetSkillEnabled { path, enabled } => {
                    ...
                    .await
                {
                    Ok(()) => {
                        ...
                        if let Err(err) = self
                            .refresh_in_memory_config_from_disk_for_discovery_cwd(discovery_cwd)
                            .await
                        {
                            tracing::warn!(
                                error = %err,
