...
async fn read_session_cwd(
    ...
    let mut rollout_error = None;
    let rollout_state = match path {
        Some(path) => match read_rollout_resume_state(path).await {
            Ok(state) => Some(state),
            Err(err) => {
                rollout_error = Some((path.display().to_string(), err));
                None
            }
        },
        None => None,
    ...
    if rollout_state
        .as_ref()
        .is_some_and(|state| state.cwd_from_turn_context)
    {
        return rollout_state.and_then(|state| state.cwd);
    }
    ...
    if let Some(state) = rollout_state {
        return state.cwd;
    }

    if let Some((rollout_path, err)) = rollout_error {
        tracing::warn!(
            %rollout_path,
            %err,
        ...
        match record.item_type.as_str() {
            ...
            "turn_context" => {
                if let Ok(turn_context) = serde_json::from_value::<TurnContextResumeState>(payload)
                {
                    ...
                    state.cwd_from_turn_context = true;
...
mod tests {
    ...
    #[tokio::test]
    async fn read_session_cwd_prefers_rollout_turn_context_over_state_db() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let thread_id = ThreadId::new();
        let rollout_cwd = temp_dir.path().join("rollout-cwd");
        let state_db_cwd = temp_dir.path().join("state-db-cwd");
        std::fs::create_dir_all(&rollout_cwd)?;
        std::fs::create_dir_all(&state_db_cwd)?;
        let rollout_path = temp_dir.path().join("rollout.jsonl");
        write_rollout_lines(
            &rollout_path,
            &[rollout_line(
                "t0",
                "turn_context",
                serde_json::json!({ "cwd": rollout_cwd.clone(), "model": "latest" }),
            )],
        )?;

        let runtime =
            StateRuntime::init(temp_dir.path().join("codex-home"), "test-provider".to_string())
                .await
