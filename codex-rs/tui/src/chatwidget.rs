...
pub(crate) struct ChatWidget {
    ...
    pending_file_change_changes: HashMap<String, HashMap<PathBuf, FileChange>>,
    replayed_file_change_starts: HashSet<String>,
...
impl ChatWidget {
    ...
    fn on_session_configured_with_display_and_fork_parent_title(
        ...
        self.pending_file_change_changes.clear();
        self.replayed_file_change_starts.clear();
    ...
    fn on_patch_apply_begin(
        ...
        let display_changes = file_update_changes_to_display(changes);
        self.pending_file_change_changes
            .insert(id.clone(), display_changes.clone());
        if capture_snapshots {
            self.capture_pending_patch_snapshots(&id, &display_changes);
        }
    ...
    fn on_file_change_completed(&mut self, item: ThreadItem) {
        let ThreadItem::FileChange { id, status, .. } = &item else {
            return;
        };
        self.pending_file_change_changes.remove(id);
        self.finish_pending_patch_snapshots(id, status);
        let item2 = item.clone();
        self.defer_or_handle(
    ...
    fn capture_pending_patch_snapshots(
        &mut self,
        call_id: &str,
        changes: &HashMap<PathBuf, FileChange>,
    ) {
    ...
    fn handle_command_execution_completed_now_with_runtime_cwd_sync(
        &mut self,
        item: ThreadItem,
    ...
    fn new_with_op_target(common: ChatWidgetInit, codex_op_target: CodexOpTarget) -> Self {
        ...
        let mut widget = Self {
            ...
            pending_file_change_changes: HashMap::new(),
            replayed_file_change_starts: HashSet::new(),
    ...
    fn handle_thread_item(
        ...
        match item {
            ...
            ThreadItem::ContextCompaction { summary, .. } => self.on_context_compacted(summary),
    ...
    fn handle_item_started_notification(
        ...
        match notification.item {
            ...
            ThreadItem::FileChange { id, changes, .. } => {
                ...
                self.on_patch_apply_begin(id, changes, !from_replay);
            }
    ...
    fn handle_item_completed_notification(
            ...
            } = &notification.item
        {
            ...
            ) && !self.replayed_file_change_starts.remove(id)
            {
                self.on_patch_apply_begin(
                    id.clone(),
                    changes.clone(),
